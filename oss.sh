...
for i in {0..9};
do
        out=$(python3 infra/helper.py run_fuzzer libpng libpng_transformation_fuzzer --corpus-dir build/out/corpus$i  &> build/out/log$i & echo $!)
        pids+=($out)

done;
echo ${pids[@]}
echo "Fuzzers are running... Let's wait 4 hours now."
for i in {1..240};
do
        sleep 1m
        ...
        for pidi in ${!pids[@]};do
                if  ! [ -d "/proc/${pids[$pidi]}" ]; then
                        out=$(python3 infra/helper.py run_fuzzer libpng libpng_transformation_fuzzer --corpus-dir build/out/corpus${pidi}  &>> build/out/log${pidi} & echo $!)
                        pids[${pidi}]=$out
                fi
        done
done
